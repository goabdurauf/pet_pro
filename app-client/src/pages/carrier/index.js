...
@connect(({carrier, app}) => ({carrier, app}))
class Carrier extends Component {
  render() {
    const {carrier, dispatch} = this.props;
    const {model, isModalOpen, itemList, currentItem, modalType, countryList, managerList, aboutList, visibleColumns} = carrier;
    ...
    const modalProps = {
      visible: isModalOpen,
      title: modalType === 'create' ? 'Создать перевозчика' : 'Редактировать перевозчика',
      onCancel() {
        dispatch({
          type: model + '/updateState',
          payload: {
    ...
    return (
      <div className="users-page">
        <Card style={{width: '100%'}} bordered={false}>
          <Row>
            <Col span={4}><Typography.Title level={4}>Перевозчики</Typography.Title></Col>
